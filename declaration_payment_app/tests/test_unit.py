...
from declaration_payment_app import views
...
class DeclarationTests(TestCase):
    ...
    def test_link_to_master_summary_page_when_completed_tasks(self):
        ...
        with mock.patch('identity_models.user_details.UserDetails.api.get_record') as identity_api_get, \
            ...
            self.assertContains(
                response,
                '<a href="{}">'.format(
                    reverse('declaration-payment:Master-Summary') + '?id=' + self.application_id),
                # '<a href="{}"><span class="task-name">Declaration and Payment</span></a>'.format(reverse('declaration-payment:Master-Summary') + '?id=' + self.application_id),
                html=True
            )
    ...
    def test_can_render_master_summary_page(self):
        ...
        with mock.patch('identity_models.user_details.UserDetails.api.get_record') as identity_api_get, \
            ...
            # TODO: Check which mocks need calling.
            ...
            response = self.client.get(reverse('declaration-payment:Master-Summary') + '?id=' + self.application_id)
    ...
    def test_post_request_to_master_summary_redirects(self):
        ...
        """
        response = self.client.post(reverse('declaration-payment:Master-Summary') + '?id=' + self.application_id)
    ...
    def test_declaration_guidance_page_can_be_rendered(self):
        ...
        Test to assert that the 'Master-Summary' page can be rendered.
        """
        response = self.client.get(reverse('declaration-payment:Declaration-Guidance') + '?id=' + self.application_id)
        found = resolve(response.request.get('PATH_INFO'))
    ...
    def test_final_declaration_page_can_be_reviewd(self):
        ...
        """
        response = self.client.get(reverse('declaration-payment:Declaration-Summary') + '?id=' + self.application_id)
        found = resolve(response.request.get('PATH_INFO'))
    ...
    def test_can_complete_final_declaration(self):
        ...
        """
        response = self.client.post(reverse('declaration-payment:Declaration-Summary') + '?id=' + self.application_id,
                                    {
                                        'follow_rules': True,

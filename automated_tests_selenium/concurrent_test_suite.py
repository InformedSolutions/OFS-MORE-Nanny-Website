...
class ConcurrentTestRunner(DiscoverRunner):
    """
    Test suite for running Selenium tests in a concurrent manner.
    Since the Selenium tests are contained within a single TestCase class, the Django ParallelTestSuite will be unable
    to operate in an optimal manner.
    This, in conjunction with the fact that our servers operate on a single core, motivate the use of a
    ConcurrentTestRunner for the specific case of Selenium tests, in which individual tests within a TestCase class are
    each run in an individual thread.
    """
    def run(self, test):
        """
        Run the given test case, opening a new thread for each contained test.
        """
        ...
        for thread in range(N_THREADS):
            test_thread = threading.Thread(
                group=None,
                target=XMLTestRunner(
                    verbosity=self.verbosity, descriptions=self.descriptions,
                    output=self.output, failfast=self.failfast).run,
                name=None,
                args=[test_queue.get()]
            )
            test_thread.start()
    ...
    def run_suite(self, suite, **kwargs):
        ...
        if isinstance(verbosity, bool):
            self.verbosity = (1, 2)[verbosity]
        self.descriptions = getattr(settings, 'TEST_OUTPUT_DESCRIPTIONS', False)
        self.output = getattr(settings, 'TEST_OUTPUT_DIR', '.')
        return ConcurrentTestRunner(
            verbosity=self.verbosity, descriptions=self.descriptions,
            output=self.output, failfast=self.failfast).run(suite)

    def __init__(self, *args, **kwargs):
        self.descriptions = kwargs.pop('descriptions', None)
        self.output = kwargs.pop('output', None)
        super(ConcurrentTestRunner, self).__init__(*args, **kwargs)
